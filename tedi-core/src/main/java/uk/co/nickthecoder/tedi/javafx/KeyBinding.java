...
public class KeyBinding {
    ...
    public int getSpecificity(Control control, KeyEvent event) {
        ...
        else if (shift != OptionalBoolean.ANY) s++;
        if (!ctrl.equals(event.isControlDown())) return 0;
        else if (ctrl != OptionalBoolean.ANY) s++;
        if (!alt.equals(event.isAltDown())) return 0;
        else if (alt != OptionalBoolean.ANY) s++;
        if (!meta.equals(event.isMetaDown())) return 0;
        else if (meta != OptionalBoolean.ANY) s++;
        if (eventType != null && eventType != event.getEventType()) return 0;
        else s++;
